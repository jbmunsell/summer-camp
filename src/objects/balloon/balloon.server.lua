...
local balloonConfig = require(balloon.config)
...
local balloonObjectStream = objectsUtil.initObjectClass(balloon)

-- Place on request
pickupUtil.getPlayerObjectActionRequestStream(
	balloon.net.PlacementRequested,
	balloonConfig.instanceTag
...
balloonObjectStream
	:flatMap(function (balloonInstance)
		return rx.Observable.fromProperty(balloonInstance.Balloon, "Position")
			:filter(function (position) return position.Y > balloonConfig.DestroyHeight end)
			:map(dart.constant(balloonInstance))
	end)
