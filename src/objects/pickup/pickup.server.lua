...
local pickupInstanceStream = objectsUtil.initObjectClass(pickup)
...
pickupInstanceStream
	:flatMap(function (instance)
		if not objectsUtil.getConfig(instance).pickup then
			warn("No pickup config for " .. instance:GetFullName())
		end
		local interactEnabled = objectsUtil.getConfig(instance).pickup.interactPickupEnabled
...
local touchPickupStream = pickupInstanceStream
	:filter(function (instance)
		return objectsUtil.getConfig(instance).pickup.touchPickupEnabled
	end)
...
throwStream
	:map(function (player, target)
		...
		local config = (throwObject and objectsUtil.getConfig(throwObject))
		local throwPower = (config and config.pickup.throwMagnitude)
...
local characterEquipStream = playerEquipStream
	...
	:filter(function (character, instance)
		return CollectionService:HasTag(instance, pickupConfig.instanceTag)
