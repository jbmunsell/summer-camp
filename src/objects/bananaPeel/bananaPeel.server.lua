...
local bananaPeel = objects.bananaPeel
...
local function destroyBananaPeel(peel)
	...
	delay(bananaPeelConfig.destroyFadeDuration + 1, function ()
		peel:Destroy()
	end)
end
...
local peelObjectStream = objectsUtil.initObjectClass(bananaPeel)
...
local characterTripStream = peelObjectStream
	:delay(0.1)
	:flatMap(function (peel)
		return rx.Observable.fromPlayerTouchedDescendant(peel, bananaPeelConfig.peelDebounce)
	end)
