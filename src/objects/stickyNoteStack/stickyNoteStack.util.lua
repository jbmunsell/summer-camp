...
function stickyNoteStackUtil.getAttachmentCFrame(raycastData)
	return raycastData.instance.CFrame:toObjectSpace(stickyNoteStackUtil.getWorldCFrame(raycastData))
end

-- Get world cframe
function stickyNoteStackUtil.getWorldCFrame(raycastData)
	return CFrame.new(raycastData.position, raycastData.position + raycastData.normal)
		* stickyNoteStackConfig.stickProtrusion
		* stickyNoteStackConfig.stickAngle
...
function stickyNoteStackUtil.stickNote(note, raycastData)
	...
	stickAttachment.CFrame = stickyNoteStackUtil.getAttachmentCFrame(raycastData)
...
function stickyNoteStackUtil.createNote(stack, raycastData, text)
	local note = env.res.objects.StickyNote:Clone()
