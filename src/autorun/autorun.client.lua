...
local running = rx.Observable.from(env.LocalPlayer.CharacterAdded)
	...
	:share()
running:filter():subscribe(function ()
	-- Speed them up after a few seconds if we're still running
	local stopped = running:reject()
	local fast = rx.Observable.timer(AutorunTimer):takeUntil(stopped)
	local slow = stopped:first()
	fast:map(dart.constant(true))
		:merge(slow:map(dart.constant(false)))
