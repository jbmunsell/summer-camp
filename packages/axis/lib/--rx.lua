...
util.boolify = function (v) return v and true or false end
util.boolNot = function (b) return not b end
util.boolAnd = function (...)
  for _, v in ipairs(util.pack(...)) do
...
util.carry = function (...)
  local load = util.pack(...)
  return function (...)
    local args = util.pack(...)
    for i, l in ipairs(load) do
      table.insert(args, i, l)
    end
    return util.unpack(args)
  end
end
util.drag = function (...)
  local load = util.pack(...)
  return function (...)
    local args = util.pack(...)
    for _, l in ipairs(load) do
      table.insert(args, l)
    end
    return util.unpack(args)
  end
end
util.omitFirst = function (_, ...)
  return ...
end
util.bind = function (f, ...)
  local base = util.pack(...)
  return function(...)
    local args = util.pack(...)
    for i, arg in ipairs(base) do
      table.insert(args, i, arg)
    end
    return f(util.unpack(args))
  end
end
util.equals = function (val)
  return function (x)
    return x == val
  end
end
util.notEquals = function (val)
  return function (x)
    return x ~= val
  end
end
util.isLuaClass = function (object, class)
  local meta = (type(object) == 'table' and getmetatable(object))
  return meta and meta.__index == class
end
...
-- Roblox utils
util.isa = function (class)
  return function (instance)
    return instance:IsA(class)
  end
end
util.printConstant = function (str)
  return function ()
    print(str)
  end
end
util.getPlayerFromCharacterChild = function (child)
  return child.Parent and game:GetService("Players"):GetPlayerFromCharacter(child.Parent)
end
util.callMethod = function (methodName)
  return function (instance, ...)
    return instance[methodName](instance, ...)
  end
end
util.setProperty = function (instance, property)
  return function (value)
    instance[property] = value
  end
end
util.hasChild = function (childName)
  return function (instance)
    return instance:FindFirstChild(childName)
  end
end
util.hasTag = function (tagName)
  return function (instance)
    return game:GetService("CollectionService"):HasTag(instance, tagName)
  end
end
util.destroy = function (child)
  child:Destroy()
end
...
function Observer:onError(message)
  if not self.stopped then
    ...
    print("Attempt to error on an observer that already stopped")
  end
end
...
function Observable:subscribe(onNext, onError, onCompleted)
  -- local observer = (type(onNext) == "table" and onNext or Observer.create(onNext, onError, onCompleted))
  local observer
  if type(onNext) == "table" then
    -- if util.isLuaClass(onNext, Observer) then
      observer = onNext
    -- else
      -- observer = Observer.create(onNext.onNext, onNext.onError, onNext.onCompleted)
    -- end
  else
    observer = Observer.create(onNext, onError, onCompleted)
  end
  local subscription = self._subscribe(observer)
...
-- Roblox specific axis additions
do
  -- Grab services
  ...
  --- Creates an Observable that propogates values from an RBXScriptSignal object
  function Observable.fromEvent(rbxevent)
    assert(rbxevent, "RBXScriptSignal expected as arg #1 to Observable.fromEvent")
    return Observable.create(function (observer)
      local rbxconnection = rbxevent:Connect(function(...)
        util.tryWithObserver(observer, function (...)
          return observer:onNext(...)
        end, ...)
      end)
      -- print("rx connected to roblox event")

      return Subscription.create(function ()
        -- print("rx disconnected an observed roblox event")
  ...
  function Observable.fromInstanceLeftGame(instance)
    return Observable.fromEvent(instance.AncestryChanged)
      :filter(function () return not instance:IsDescendantOf(game) end)
      :first()
  end
  function Observable.fromInstanceEvent(instance, eventName)
    local eventObservable = Observable.fromEvent(instance[eventName])
    return eventObservable:takeUntil(Observable.fromInstanceLeftGame(instance))
  end

  function Observable.fromValueObject(object, startWith)
    local obs = Observable.fromInstanceEvent(object, "Changed")
    if startWith then
      return obs:startWith(object.Value)
    else
      return obs
    end
  end
  function Observable.fromHeartbeat()
    return Observable.fromEvent(RunService.Heartbeat)
  end
  function Observable.fromPlayerAdded()
    return Observable.fromTable(Players:GetPlayers())
      :merge(Observable.fromEvent(Players.PlayerAdded))
  end
  function Observable.fromPlayerCharacterAdded()
    return Observable.fromPlayerAdded()
      :flatMap(function (player)
        return Observable.fromEvent(player.CharacterAdded)
          :startWith(player.Character)
          :filter()
          :map(util.carry(player))
      end)
  end
  function Observable.fromInstanceTag(tag)
    return Observable.fromTable(CollectionService:GetTagged(tag))
      :merge(Observable.fromEvent(CollectionService:GetInstanceAddedSignal(tag)))
  end
  function Observable.fromInstanceProperty(instance, property)
    return Observable.fromEvent(instance:GetPropertyChangedSignal(property))
      :map(function (value) return instance, value end)
  end
  function Observable.fromInterval(interval)
    ...
    return Observable.fromEvent(bindable.Event)
  end

  function Observable.fromKey(keyCode)
    local keyEvent = Instance.new("BindableEvent")
    local identifier = string.format("observed_key_%s_%f", tostring(keyCode.Value), os.clock())
    ContextActionService:BindAction(identifier, function (_, state, _)
      keyEvent:Fire(state)
    end, false, keyCode)
    return Observable.fromEvent(keyEvent.Event)
  end
...
-- Observable boolean operators
function Observable:boolNot()
  return self:map(util.boolNot)
end
function Observable:boolAnd(...)
  local args = util.pack(...)
  table.insert(args, util.boolAnd)
  return self:combineLatest(util.unpack(args))
end
    ...
    local function onNext(i)
      return function(value)
        ...
        if not next(pending) then
          util.tryWithObserver(observer, function()
            observer:onNext(combinator(util.unpack(latest, 1, #sources)))
          end)
        end
      end
    end
...
-- Debounce front
--  Returns an observable that delivers the first event but does not deliver more events
--    until a specified time has elapsed since the previously EMITTED event
function Observable:debounceFront(dbtime)
  return Observable.create(function (observer)
    local db = false
    ...
    local function wrap(key)
      return function (...)
        ...
        db = true
        delay(dbtime, function () db = false end)

        -- Pass along
        return observer[key](observer, ...)
      end
    end
    ...
    local function onCompleted(e)
      return observer:onCompleted(e)
    end

    return self:subscribe(wrap("onNext"), onError, onCompleted)
  end)
end
...
function Observable:delay(seconds)
  return Observable.create(function (observer)
    ...
    local function onCompleted(message)
      return observer:onCompleted(message)
    end

    local subscription = self:subscribe(dl("onNext"), onError, onCompleted)
...
-- Returns a new Observable that produces values that are preceded by a given static object
-- function Observable:carry(load)
--   return self:map(function (...)
--     return load, ...
--   end)
-- end
-- function Observable:carryLatest(loadObservable)
--   return self:withLatestFrom(loadObservable):map(function (...)
--     local arg = util.pack(...)
--     local load = table.remove(arg, #arg)
--     return load, util.unpack(arg)
--   end)
-- end
...
function Observable:scanAndStart(accumulator, seed)
  return self:scan(accumulator, seed)
    :startWith(seed)
end
...
function Observable:takeUntil(other)
  return Observable.create(function(observer)
    ...
    other:first():subscribe(onCompleted, onCompleted, onCompleted)
...
function Observable:withLatestFrom(...)
  ...
  return Observable.create(function(observer)
    ...
    local function onNext(value)
      return observer:onNext(value, util.unpack(latest, 1, #sources))
    end
...
BehaviorSubject.push = BehaviorSubject.onNext
...
-- Share
--  Creates a new Subject and forwards values from the source observable
function Observable:share()
  local subject = Subject.create()
  return subject, self:subscribe(subject)
end

-- Share behavior
--  Creates a new BehaviorSubject and forwards values from the source observable
function Observable:shareBehavior()
